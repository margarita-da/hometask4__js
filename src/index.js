...
function findAllPSiblings(where) {
    let arr = [];
    let elems = document.where.childNodes;
...
/*
 ...
 Функция представленная ниже, перебирает все дочерние узлы типа "элемент" внутри узла переданного в параметре where и возвращает массив 
    ...
    let obj = {
      ...
      function recur(root){
        for (let i = 0; i < root.childNodes.length; i++) {
            let elem = root.childNodes[i];
            if(elem.nodeType === 3 && elem.nodeName !== 'SCRIPT') {
                obj.texts++
            }else if(elem.nodeType === 1){

                for(let i = 0; i < elem.classList.length; i++){

                    let classes = elem.classList[i];

                    if(obj.classes[classes]){
                        obj.classes[classes]++;
                    } else {
                        obj.classes[classes] = 1;
                    }
                }
                let tagsN = elem.tagName;
